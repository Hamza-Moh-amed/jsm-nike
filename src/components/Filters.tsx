...
const Filters = () => {
    ...
    const Group = ({
        ...
        <div className="border-b border-light-300 py-4">
          ...
          >
            <span>{title}</span>
            <span className="text-caption text-dark-700 border rounded-full cursor-pointer min-w-5.5">{expanded[k] ? "−" : "+"}</span>
          </button>
                ...
                {PRICES.map((p) => {
                    ...
                    return (
                        <li key={p.id} className="flex items-center gap-2">
                            ...
                            id={`price-${p.id}`}
                            type="checkbox"
                            className="w-4 h-4 accent-dark-900"
                            checked={checked}
                            onChange={() => onToggle("price", p.id)}
                            />
                            <label htmlFor={`price-${p.id}`} className="text-body">{p.label}</label>
                        </li>
                    )
                })}
            </ul>
